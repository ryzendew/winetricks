...
impl VerbCategory {
    ...
    pub fn from_str(s: &str) -> Option<Self> {
        match s {
            ...
            "download" => Some(VerbCategory::Download),
            "manual-download" => Some(VerbCategory::ManualDownload),
            _ => None,
        }
    }
}
...
impl VerbRegistry {
    ...
    pub fn load_from_dir<P: AsRef<Path>>(dir: P) -> Result<Self> {
        ...
        for category_dir in std::fs::read_dir(dir)? {
            ...
            let category = VerbCategory::from_str(category_name)
                .ok_or_else(|| WinetricksError::Verb(format!("Unknown category: {}", category_name)))?;
            ...
            for entry in std::fs::read_dir(&path)? {
                ...
                let verb_name = file_path.file_stem()
                    .and_then(|n| n.to_str())
    ...
    /// Register a verb
    pub fn register(&mut self, name: String, metadata: VerbMetadata, category: VerbCategory) -> Result<()> {
        if self.verbs.contains_key(&name) {
            return Err(WinetricksError::Verb(format!("Verb '{}' already registered", name)));
        }
