...
use crate::verb::{VerbMetadata, VerbRegistry, VerbCategory};
use crate::wine::Wine;
...
impl Executor {
    ...
    pub async fn install_verb(&mut self, verb_name: &str) -> Result<()> {
        ...
        std::env::set_var("W_OPT_UNATTENDED", if self.config.unattended { "1" } else { "0" });
        ...
        let metadata = match self.registry.get(verb_name) {
            ...
            None => {
                ...
                if let Some(ref script) = original_script {
                    warn!("Verb '{}' not in Rust metadata, delegating to original winetricks", verb_name);
                    return self.delegate_to_original_winetricks(script, verb_name).await;
                }
        ...
        for file in &metadata.files {
            if let Some(ref url) = file.url {
                ...
                let _downloaded = self.downloader
                    .download(url, &cache_dir.join(&file.filename), file.sha256.as_deref(), true)
                    .await?;
            }
        }
        ...
        self.execute_verb_installation(&metadata, &cache_dir).await?;
        ...
        if duration_secs >= 60 {
            ...
            println!("Successfully installed {} in {}m {}.{:03}s", verb_name, minutes, seconds, duration_millis);
        } else {
            println!("Successfully installed {} in {}.{:03}s", verb_name, duration_secs, duration_millis);
        }
        ...
        for file in &files {
            ...
            match ext {
                ...
                "exe" => {
                    ...
                    let is_dotnet = filename.contains("dotnet") || filename.contains("ndp") || filename.starts_with("NDP");
                    let is_vcredist = filename.contains("vcredist") || filename.contains("vc_redist");
                    let is_ie = filename.contains("IE") || filename.contains("ie") || filename.contains("internetexplorer");
                    ...
                    if self.config.unattended {
                        if is_dotnet {
                            ...
                            if filename.contains("48") || filename.contains("472") || filename.contains("46") || filename.contains("462") {
                ...
                "zip" => {
                    ...
                    return Err(WinetricksError::Verb("ZIP extraction not yet implemented".into()));
                }
                "cab" => {
                    info!("Extracting cab: {:?}", file);
                    // TODO: Extract cab to wineprefix using cabextract
                    return Err(WinetricksError::Verb("CAB extraction not yet implemented".into()));
                }
                _ => {
    ...
    fn unix_to_wine_path(&self, unix_path: &Path) -> Result<String> {
        ...
        if output.status.success() {
            ...
            Ok(format!("Z:\\{}", unix_path.to_string_lossy().replace('/', "\\")))
        }
    }
    ...
    async fn delegate_to_original_winetricks(&self, script_path: &Path, verb_name: &str) -> Result<()> {
        let start_time = Instant::now();
        ...
        if status.success() {
            ...
            if duration_secs >= 60 {
                let minutes = duration_secs / 60;
                let seconds = duration_secs % 60;
                println!("Successfully installed {} (via original winetricks) in {}m {}.{:03}s", verb_name, minutes, seconds, duration_millis);
            } else {
                println!("Successfully installed {} (via original winetricks) in {}.{:03}s", verb_name, duration_secs, duration_millis);
            }
            Ok(())
        } else {
